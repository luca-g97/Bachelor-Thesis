...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			{
				// is cc needed?
				rgAjaxParams = { /*cc: rgHoverData['cc'],*/ l: 'german' };
			}
			if ( rgHoverData['v6'] )
			{
...
{
	if ( nCount == 0 )
		$JFromIDOrElement('wishlist_link').text( 'Wunschliste' );
	else
		$JFromIDOrElement('wishlist_link').html( 'Wunschliste (<span id="wishlist_item_count_value">' + nCount.toString() + '</span>)' );
}
...
function AddToWishlist( appid, divToHide, divToShowSuccess, divToShowError, navref, divToHide2 )
{
	if ( !g_AccountID )
	{
		ShowAlertDialog( "Bitte anmelden\u00a0\u2026", "Bitte melden Sie sich mit Ihrem Steam-Account an, um einen Artikel Ihrer Wunschliste hinzuzuf\u00fcgen." )
		.done( function() {
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'Wunschliste erweitern' );
				}
			}
			else {
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			if ( data && data.success )
			{
				...
				$J('span',button).text("Auf Ihrer Wunschliste");
				UpdateWishlistCount( data.wishlistCount );
			}
			else
			{
				$J('span',button).text( "Ups, sorry!" );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'https://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">Okay, wir werden nächstes Mal etwas anderes empfehlen</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'german'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'https://store.steampowered.com/tag/de/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.when.apply( $J, posts ).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s wurde zu Ihrem Account hinzugefügt und ist nun in Ihrer Steam-Bibliothek verfügbar.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Dieses Produkt befindet sich bereits in Ihrer Steam-Bibliothek.' );
		else if ( data && data.purchaseresultdetail == 24 )
			ShowAlertDialog( strDisplayName, 'Bevor Sie Ihrem Steam-Account dieses Produkt hinzufügen können, müssen Sie das Basisprodukt besitzen.' );
		else
			ShowAlertDialog( strDisplayName, 'Beim Hinzufügen dieses Produkt zu Ihrem Account ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Sprache ändern', 'Beim Speichern der Änderungen ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.' );
	});
}
...
function ShowGamePurchaseDropdown( elemLink, elemPopup )
{
	...
	if ( window.UseTabletScreenMode && window.UseTabletScreenMode() )
	{
		...
		var $Dialog = ShowDialog( 'Kaufoption auswählen', $Content.show() );
		$Dialog.AdjustSizing();
	}
	else
	{
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Melden Sie sich bitte an", "Sie m\u00fcssen angemeldet sein, um einem Kurator zu folgen");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Fehler', 'Beim Folgen des Steam-Kurators ist ein Fehler aufgetreten. Mögliche Ursachen<ul><li>Sie können keinem Kurator folgen, den Sie ignorieren.</li><li>Sie können nur bis zu 100 Kuratoren zur selben Zeit folgen.</li><li>Eingeschränkte Accounts können Kuratoren nicht folgen.</li></ul>' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Bitte anmelden", "Sie m\u00fcssen sich anmelden, um einen Steam-Kurator ignorieren zu k\u00f6nnen.");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Fehler', 'Beim Versuch diesen Steam-Kurator zu ignorieren, ist ein Fehler aufgetreten.<br>Hinweis: Sie können keinen Kurator ignorieren, dem Sie folgen.' );
		}
	);
	return false;
}
