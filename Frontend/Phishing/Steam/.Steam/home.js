...
GHomepage = {
	...
	BuildMainCapsuleItem: function( rgItem, strFeatureContext, nDepth )
	{
		...
		if( rgRecommendationReasons.recommended_by_friend )
		{
			...
			var $ReasonMain = $J('<div/>').addClass('main').addClass('friend').html( "<strong>Empfohlen<\/strong> von Ihrem Freund <span>%1$s<\/span>".replace("%1$s", friend.name ) );
		...
		else if( rgRecommendationReasons.recent_release_by_creator )
		{
			...
			var $ReasonLocal = creator.link.indexOf( 'developer/' ) >= 0 ? "<strong>Entwickelt<\/strong> von<br><span>%1$s<\/span>" : "<strong>Herausgegeben<\/strong> von<br><span>%1$s<\/span>";
		...
		else if( rgRecommendationReasons.recommended_by_curator )
		{
			...
			var $ReasonMain = $J('<div/>').addClass('main').addClass('curator').html( "<strong>Empfohlen<\/strong> von<br><span>%1$s<\/span>".replace("%1$s", V_EscapeHTML( curator.name ) ) );
		...
		else if( rgRecommendationReasons.recommended && GDynamicStore.s_rgRecommendedTags )
		{
			...
			if( rgMatchedTags.length == 0 )
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Empfohlen<\/strong>, da Sie \u00e4hnliche Spiele spielen" );
			else
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Empfohlen<\/strong>, weil Sie Spiele mit den folgenden Tags gespielt haben:" );
		...
		else
		{
			...
			{
				strStatus = 'Early Access - jetzt verfügbar';
			}
			else if ( rgItemData.popular_new_on_steam )
			{
				strStatus = 'Neu bei Steam';
			}
			...
			{
				strStatus = 'Jetzt vorbestellen';
			}
			else if ( rgItemData && rgItemData.video )
			{
				strStatus = 'Jetzt zum Anschauen verfügbar';
			}
		...
		if( bShowAdditionalReasons )
		{
			...
			for( var key in rgRecommendationReasons )
			{
				...
				if( key == 'top_seller' )
					...
					strReason = "Neuerscheinungen";
				else if( key == 'recommended')
					strReason = "\u00c4hnlich wie Spiele, die Sie spielen";
				else
					continue;
				...
				$el.on('hover focus', function(){
					...
					else
					{
						...
						if ( rgData['review_summary'] )
						{
							...
							$elReviewData.append( $J('<div>', {'class': 'title'}).text( rgData.appids ? 'Nutzerrezensionen für Artikel in diesem Bündel:' : 'Gesamte Nutzerrezensionen:') );
						...
						if( rgData.tags )
						{
							...
							{
								var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/de/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab_preview' );
						...
						for( var i=0; i < 4; i++ )
						{
							...
							{
								var $elScreenshot = $J('<div>', {'class': 'screenshot empty'}).append("<div>").text("Dieser Screenshot enth\u00e4lt m\u00f6glicherweise nicht jugendfreie Inhalte.");
								$elInfoDiv.append($elScreenshot);
							}
						}
	...
	RenderRecommendedByDeepDiveCarousel: function()
	{
		...
		focusedAppTitle = 'Weil Sie <strong>%s</strong> gespielt haben'.replace( "%s", focusedAppTitle);
	...
	RenderCommunityRecommendations: function()
	{
		...
		{
			elemPrefLastUpdated.text( 'Am %1$s gespeicherte Einstellungen werden verwendet.'.replace( "%1$s", GHomepage.strCommunityRecommendationsPrefLastSaved ) );
		}
	...
	FillPagedCapsuleCarousel: function( rgCapsules, $elTarget, fnCapsule, strNavContext, nCapsules )
	{
		...
		for( var j=0; j<rgCapsules.length; j+=nCapsules )
		{
			...
			for( var k=0; k < nCapsules && k + j < rgCapsules.length; k++ )
			{
				...
				{
					const panelString = '{"onOptionsActionDescription":"Alle anzeigen","onOptionsButton":"window.location=\'%1$s\'"}';
	...
	BuildCommunityRecommendationCap: function( strFeatureContext, oItem )
	{
		...
		if ( numReviews > 1 )
		{
			...
			let strNumReviews = '%1$s von %2$s Rezensionen'.replace( "%1$s", 1 ).replace( "%2$s", numReviews );
			...
			arrowLeft.click( function() {
				...
				var strNumReviews = '%1$s von %2$s Rezensionen'.replace( "%1$s", curIdx + 1 ).replace( "%2$s", numReviews );
		...
		for( var i = 0; i < rgReviews.length; ++i)
		{
			...
			var $ViewMore = $J( '<div class="view_more"><a href="' + review.review_link + '" target="_blank">Gesamte Rezension lesen</a></div>' );
			...
			if ( reviewer )
			{
				...
				{
					var strHours = '%s Stunden Spielzeit zum Zeitpunkt der Rezension'.replace( "%s", v_numberformat( review.playtime_at_review / 60, 1 ) );
					$AuthorDetails.append( $J( '<div>', { class: "hours ellipsis", text: strHours } ) );
				}
				if ( review.votes_up > 0 )
				{
					var voteUpText = '1 Person fand diese Rezension hilfreich';
					if ( review.votes_up > 1 )
					{
						voteUpText = '%2$s Personen fanden diese Rezension hilfreich'.replace( "%2$s", v_numberformat( review.votes_up ) );
					}
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/de/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
...
{
	var $Btn = $J('<div/>', {'class': 'home_btn home_customize_btn' } ).text( 'Anpassen' );
...
CHomeSettings.prototype.DisplayPopup = function( $Btn )
{
	...
	this.m_$Popup.append( $J('<div/>', {'class': 'home_viewsettings_instructions' } ).text( 'Wählen Sie die Produkttypen aus, die Sie in diesem Bereich angezeigt bekommen möchten' ) );

	if ( this.m_ApplicableSettings.popular_new_releases )
		this.m_$Popup.append( this.RenderCheckbox( 'popular_new_releases', 'Beliebte Neuerscheinungen' ) );
	if ( this.m_ApplicableSettings.top_sellers )
		this.m_$Popup.append( this.RenderCheckbox( 'top_sellers', 'Topseller' ) );
	if ( this.m_ApplicableSettings.early_access )
		this.m_$Popup.append( this.RenderCheckbox( 'early_access', 'Early-Access-Produkte' ) );
	if ( this.m_ApplicableSettings.games_already_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_already_in_library', 'Spiele, die Sie bereits besitzen' ) );
	if ( this.m_ApplicableSettings.games_not_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_not_in_library', 'Spiele, die nicht in Ihrem Account sind' ) );
	if ( this.m_ApplicableSettings.recommended_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_for_you', 'Für Sie empfohlen' ) );
	if ( this.m_ApplicableSettings.prepurchase )
		this.m_$Popup.append( this.RenderCheckbox( 'prepurchase', 'Vorbestellen' ) );
	if ( this.m_ApplicableSettings.games )
		this.m_$Popup.append( this.RenderCheckbox( 'games', 'Spiele' ) );
	...
	if ( this.m_ApplicableSettings.dlc_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc_for_you', 'Zusatzinhalte für Ihre Spiele & Software' ) );
	if ( this.m_ApplicableSettings.recently_viewed )
		this.m_$Popup.append( this.RenderCheckbox( 'recently_viewed', 'Kürzlich aufgerufene Produkte' ) );
	if ( this.m_ApplicableSettings.new_on_steam )
		this.m_$Popup.append( this.RenderCheckbox( 'new_on_steam', 'Neu bei Steam' ) );
	if ( this.m_ApplicableSettings.dlc )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc', 'Zusatzinhalte' ) );
	...
	if ( this.m_ApplicableSettings.localized )
		this.m_$Popup.append( this.RenderCheckbox( 'localized', 'Spiele in meiner Sprache' ) );
	if ( this.m_ApplicableSettings.virtual_reality )
		this.m_$Popup.append( this.RenderCheckbox( 'virtual_reality', 'Virtuelle Realität' ) );
	if ( this.m_ApplicableSettings.recommended_by_curators )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_by_curators', 'Von Ihren Steam-Kuratoren empfohlen' ) );

	if ( this.m_ApplicableSettings.only_current_platform )
	{
		// this one is a little magic
		if ( GDynamicStore.s_bUserOnLinux )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Verfügbar für Linux/SteamOS' ) );
		else if ( GDynamicStore.s_bUserOnMacOS )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Verfügbar für Mac' ) );
	}
...
CHomeSettings.prototype.OnCheckboxToggle = function( strSettingName, $Checkbox )
{
	...
	$J.post( 'https://store.steampowered.com/dynamicstore/updatehomeviewsettings', {
		...
		ShowAlertDialog( 'Anpassen', 'Beim Speichern Ihrer Einstellungen ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.' );
	} );
};
...
GSteamCurators = {
    ...
    BuildResponsiveHomePageGiantCap: function( strFeatureContext, oItem )
	{
        ...
        var $Status = $J('<div/>', {'class': 'recommendation_status recommended'}).text( "Empfohlen" );
