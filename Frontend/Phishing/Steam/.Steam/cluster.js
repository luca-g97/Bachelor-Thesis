...
Cluster.BuildClusterElements = function ( $ClusterCtn, rgMainCaps, strFeatureOverride )
{
	...
	for ( var i = 0; i < rgMainCaps.length; i++ )
	{
		...
		if ( oItem.recommended_by_curator )
			strStatus = 'Empfohlen von Ihren Kuratoren';
		else if ( oItem.recommended )
			strStatus = 'Für Sie empfohlen';
...
Cluster.BuildClusterCapsule = function( unAppID, unPackageID, strStatus, strFeature, nDepth, rgItemInfo )
{
	...
	{
		var $elWhyButton = $J ( '<span />' ).text ( rgItemInfo.recommended ? "Warum?" : "Wer?");
		$elWhyButton.click ( function ( event )
		{
			...
			} ).done ( function ( data )
			{
				...
				if ( data.curators )
				{
					strTooltipContent += '<div class="reasonheader">' +"Empfohlen von Kuratoren, denen Sie folgen:" +'</div>';
				...
				if ( data.tags )
				{
					strTooltipContent += '<div class="reasonheader">' + "\u00c4hnliche Tags zu Spielen, die Sie spielen:" +'</div>';
					for ( var i = 0; i < data.tags.length; i++ )
					{
						strTooltipContent += '<a href="https://store.steampowered.com/tag/de/'+data.tags[ i ].name+'/">' + data.tags[ i ].name + '</a>';
					}
				}

				if ( !strTooltipContent )
					strTooltipContent = '<div class="reasonheader">' + "Nicht genug Daten" + '</div>'
						+ '<div>' + "Spielen Sie mehr Spiele, um bessere Empfehlungen zu erhalten!" + '</div>';
