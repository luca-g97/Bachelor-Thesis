...
GDynamicStore = {
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://store.steampowered.com/account/preferences' ).text ( "Einstellungen" ).addClass( 'option' );
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.append( '<div>Kaufen Sie dieses Bündel, um %1$s %% Rabatt auf alle %2$s Artikel, die Sie noch nicht besitzen, zu erhalten!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>Kaufen Sie dieses Bündel, um %1$s %% Rabatt auf alle %2$s Artikel zu erhalten!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
...
function UpdateStoreBannerForAdditionalCartDiscount( nCartDiscount )
{
	...
	<div class="placeHolder_lunarSale2019_giftActiveBar">	\
		...
		<div class="lunar_sale_title"><img src="https://store.cloudflare.steamstatic.com/public/images/promo/lunar2019/lny2019_title_de.png"/></div> \
